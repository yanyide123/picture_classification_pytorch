...
for iteration in range(start_iter, max_iter):
        ...
        if epoch % 5 == 0 and epoch > 0:
            if cfg.GPUS > 1:
                checkpoint = {'model': model.module,
                            ...
                            'epoch': epoch}
                torch.save(checkpoint, os.path.join(save_folder, 'epoch_oil_{}.pth'.format(epoch)))
            else:
    ...
    if iteration % 20 == 0:
        print('Epoch:' + repr(epoch) + ' || epochiter: ' + repr(iteration % (epoch_size + 6)) + '/' + repr(epoch_size+6)
              + '|| Totel iter ' + repr(iteration) + ' || Loss: %.6f||' % (loss.item()) + 'ACC: %.3f ||' %(train_acc * 100) + 'LR: %.8f' % (lr))
    # 计算每个epoch的acc(根据需要更改打印的步数）
    if iteration % (epoch_size+6) == 0 and iteration > 0:
        train_epoch_acc = sum(train_acc_list[-7:]) / len(train_acc_list[-7:])
        print('Epoch:' + repr(epoch) + " || train_epoch_acc:%.3f" % (train_epoch_acc * 100))
