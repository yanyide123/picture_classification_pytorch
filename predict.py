...
def predict(model):
    ...
    if torch.cuda.is_available():
        model.cuda()
    pred_list, _id, img_path_list = [], [], []
    for i in tqdm(range(len(imgs))):
        img_path_intact = imgs[i]
        # print(img_path_intact)
        img_path = imgs[i].strip()
        # print(img_path)
        _id.append(os.path.basename(img_path).split('.')[0])
        img_path_list.append(img_path_intact)
        # print(_id)
    ...
    return _id, pred_list, img_path_list
...
if __name__ == "__main__":
    ...
    _id, pred_list, img_path_list= predict(trained_model)

    # submission = pd.DataFrame({"ID": _id, "Label": pred_list})
    # submission.to_csv(cfg.BASE_PREDICT + '{}_submission_oil14_eff.csv'
    #                   .format(model_name), index=False, header=False)

    submission_copy = pd.DataFrame({"img_path": img_path_list, "Label": pred_list})
    submission_copy.to_csv(cfg.BASE_PREDICT + '{}_submission_copy_oil14_eff.csv'.format(model_name), index=False, header=False)
